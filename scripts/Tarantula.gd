...
func main_ready()->void:
	...
	init_rotation = $Sprite/Body/Butt.rotation
	init_position = $Sprite/Body/Butt.position
	for eye in $Sprite/Body/Head/eyes.get_children():
		init_eye_scale.append(eye.scale) 
	.main_ready()
	
#simple display function that multiplies the size of our eyes
func multEyeSize(factor):
	for node in $Sprite/Body/Head/eyes.get_children():
		node.scale *= factor
#this function resets the eye size to initial scale
func resetEyeSize():
	var children = $Sprite/Body/Head/eyes.get_children()
	...
	match mode:
		...
		"Attack_Alert":
			match inner_beat:
				0:
					$NotePlayer.play_note(1)
					$Sprite/Body/Butt.rotation = init_rotation - PI/10
					$Sprite/Body/Butt.position = init_position
				1:
				...
				3:
					...
					$Sprite/Body/Butt.rotation = init_rotation
					$Sprite/Body/Butt.position = Vector2(init_position.x,init_position.y+10)
				4:
		...
		"Alert":
			match inner_beat:
				0:
					$NotePlayer.play_note(0)
					$Sprite/Body/Butt.position = Vector2(init_position.x,init_position.y-5)
				1:
					$NotePlayer.play_note(7)
					$Sprite/Body/Butt.position = init_position
				2:
